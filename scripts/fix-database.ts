...
async function fixDatabase() {
  ...
  try {
    ...
    if (badgesExist.length === 0) {
      ...
      console.log("⏭️ Badges already exist, skipping...");
    }
    ...
    if (sessionsExist.length === 0) {
      ...
      console.log("✅ Live sessions added successfully");
    } else {
      console.log("⏭️ Live sessions already exist, skipping...");
    }
    ...
    if (quizzesExist.length === 0) {
      ...
      await db.insert(quizQuestions).values(jsQuestionsData);
      
      // Questions for Advanced JavaScript Concepts
      const advJsQuestionsData = [];
      for (let i = 0; i < 10; i++) {
        advJsQuestionsData.push({
      ...
      await db.insert(quizQuestions).values(advJsQuestionsData);
      
      // Questions for React Components and Props
      const reactQuestionsData = [];
      for (let i = 0; i < 10; i++) {
        reactQuestionsData.push({
      ...
      await db.insert(quizQuestions).values(reactQuestionsData);
      
      // Questions for Node.js Basics
      const nodeQuestionsData = [];
      for (let i = 0; i < 10; i++) {
        nodeQuestionsData.push({
      ...
      await db.insert(quizQuestions).values(nodeQuestionsData);
      
      // Add questions for remaining quizzes
      // CSS Flexbox and Grid
      const cssQuestionsData = [];
      for (let i = 0; i < 10; i++) {
        cssQuestionsData.push({
      ...
      await db.insert(quizQuestions).values(cssQuestionsData);
    ...
    if (challengesExist.length === 0) {
      ...
      console.log("✅ Compiler challenges added successfully");
    } else {
      console.log("⏭️ Compiler challenges already exist, skipping...");
    }
